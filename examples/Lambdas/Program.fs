...
module Parse =
    ...
    let parse arg =
        ...
        let rec parseUse () =
            ...
            | Ident ->
                let id = bump ()
                GreenNode.Create(AstKind.USE |> astToGreen, [ GreenToken.Create(AstKind.IDENT |> astToGreen, id |> getText ) |> Token ])
                |> Node
            | _ -> parseError ()
...
let kindToName =
    Parse.astFromGreen >> sprintf "%A"

let prettyPrint tree =
    let mutable indent = 0
    let printIndent () =
        String.init indent (fun _ -> "  ")
        |> printf "%s" 
    tree
    |> Walk.walk
    |> Seq.iter (function
                 | EnterNode n ->
                    printIndent ()
                    indent <- indent + 1
                    printfn "%s@%d..%d" (n.Kind |> kindToName) n.Offset (n.Offset + n.Green.TextLength)
                 | LeaveNode n ->
                    indent <- indent - 1
                 | OnToken t ->
                    printIndent ()
                    printfn "%s@%d..%d" (t.Kind |> kindToName) t.Offset (t.Offset + t.Green.TextLength))
